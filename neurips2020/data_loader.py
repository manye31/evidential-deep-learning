...
def _load_test():
    ...
    test_depths = []
    items = np.empty([0])
    for file in os.listdir('data/depth/items'):
        items = np.append(items, os.path.splitext(file)[0])
    for item in items:
        back_dir = f'data/depth/background'
        for img in os.listdir(back_dir):
            if img.endswith('.png'):
                im = Image.open(path.join(back_dir, img))
                test_images.append(np.asarray(im))
        dp_dir = 'data/depth/depth'
        for dep in os.listdir(dp_dir):
            if dep.endswith('.png'):
                dp = Image.open(path.join(dp_dir, dep))
...
def _load_ood():
    ...
    for file in os.listdir('data/depth/items'):
        items = np.append(items, os.path.splitext(file)[0])
    for item in items:
        ood_dir = f'data/depth/ood/{item}'
        for img in os.listdir(ood_dir):
            if img.endswith('.png'):
                im = Image.open(path.join(ood_dir, img))
